...
- `status` = `planned`; preencha `planned_duration_min`, `planned_distance_km` (se fizer sentido
  pra disciplina — corrida/nado/bike de distância definida; deixe `null` em treino de força ou
  por potência/tempo sem distância-alvo) e `planned_tss`.
...
### 4) Marcar feito / pulado — **ATUALIZE a linha, nunca insira uma segunda**
Quando eu contar que fiz (ou não) um treino que já estava planejado (seja porque eu descrevi
verbalmente, ou porque você tem os dados reais via Strava/Garmin), o resultado real entra na
**MESMA linha** do treino planejado daquele dia+disciplina — nunca crie uma linha nova para o
"treino feito" ao lado da linha "planejada". Isso já causou duplicatas no calendário (dois
cards no mesmo dia para a mesma sessão) e foi corrigido manualmente uma vez — não deixe
acontecer de novo.
...
3. **Um treino feito é um `update` na linha planejada, não uma linha nova** (rotina 4) — cada
   dia+disciplina deve ter no máximo 1 linha em `workouts`.
4. **Preserve o histórico**: não apague treinos `done`.
5. Depois de gravar, me **confirme por texto** o que foi para o dashboard
