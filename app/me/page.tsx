...
import { VITALS_METRICS } from "@/lib/vitals";
...
const REGISTRY: Record<BlockId, (p: BlockProps) => React.ReactNode> = {
  ...
  // This dashboard has every metric by definition, so the series here are gated
  // on data alone.
  vitals: (p) => (
    <VitalsBlock data={p.data} metrics={VITALS_METRICS} todayISO={p.todayISO} locale={p.locale} />
  ),
